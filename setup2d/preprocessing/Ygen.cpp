			...
			if (infile.good() && line.at(0) != '#')
			{
				...
				while ((pos = line.find(delimiter)) != str::npos)
				{
					...
					case(0):
					{
						...
						Geometry.getintnode(N.getnumber()).setdx(std::stod(token));
						break;
					}
					case(1):
					{
						N.setdy(std::stod(token)); Geometry.getintnode(N.getnumber()).setdy(std::stod(token));
						break;
					}
					case(2):
					{
						assert(std::stoi(token) < Property.getnummats() && "Error (in readnodes): material id invalid.\n");
						N.setmatid(std::stoi(token)); 
						Geometry.getintnode(N.getnumber()).setmatid(std::stoi(token));
						break;
					}
					case(3):
					{
						assert((std::stoi(token) == 0 || std::stoi(token) == 1) && "Error (in readnodes): boundary flag must be either 0 or 1.\n");
						N.setboundaryflag(std::stoi(token)); 
						Geometry.getintnode(N.getnumber()).setboundaryflag(std::stoi(token));
						break;
					}
