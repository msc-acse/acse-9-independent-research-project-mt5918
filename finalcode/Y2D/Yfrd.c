...
static void Yfrd2TRIANGLE(  /* fracture triangle  */
            ...
            d1nvcy,d1pefs,d1sdel,i1elpr,i1nobf,
            i1nopr,i2elto
            ) 
  ...
  DBL  *d1nvcy; DBL  *d1pefs; DBL *d1sdel; INT *i1elpr; INT *i1nobf;
  ...
  { if(i1elpr[ielem]==iprop)
    { if(d1sdel[ielem]>(RP15))
      { d1sdel[ielem]=-R1;
        ...
        ratio=d1pefs[iprop]*SQRT(MINIM(R1,(voli/volc)));
        for(i=0;i<3;i++)
        { if(nnopo>=mnopo)
...
/*********************PUBLIC********************************************************/
void Yfrd(   ydc, yde, ydi, ydn, ydp    /***  mesh elements  ***/
         )
...
{ INT nelest, nnopst;
  ...
  { if((ydp->i1ptyp[iprop]==YTE2TRISOF)&&
       (ydp->i1pefr[iprop]>0))
    { Yfrd2TRIANGLE(  /* fracture triangle  */
      ...
      ydn->d2nvc[1],ydp->d1pefs,yde->d2elst[ydp->i1psde[iprop]],
